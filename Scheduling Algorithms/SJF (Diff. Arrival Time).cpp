...
void calWaitTime(int totalProcess, Process* proc)
{
	...
	{
		timeTemp = proc[i].turnAroundTime - proc[i].burstTime;
		if (timeTemp > 0) proc[i].waitTime = timeTemp;
		else proc[i].waitTime = 0;
	}
...
void calTurnAroundtime(int totalProcess, Process* proc)
{
	...
	for (int i = 0; i < totalProcess; i++)
	{
		proc[i].turnAroundTime = proc[i].complTime - proc[i].arrivalTime;
	}
...
int main()
{
	...
	for (int i = 0; i < numOfProcess; i++)
	{
		cout << "The Waiting Time for Process with ArrivalTime,BurstTime            " << totalProc[i].processNo<< ", " 
			<< totalProc[i].arrivalTime << ", " << totalProc[i].burstTime << " is " << totalProc[i].waitTime << endl;
		cout << "The Completion Time for Process with ArrivalTime,BurstTime         " << totalProc[i].processNo<< ", " 
			<< totalProc[i].arrivalTime<< ", " << totalProc[i].burstTime << " is " << totalProc[i].complTime << endl;
		cout << "The Turn Around Time Time for Process with ArrivalTime,BurstTime   " << totalProc[i].processNo<< ", " 
			<< totalProc[i].arrivalTime << ", " << totalProc[i].burstTime << " is " << totalProc[i].turnAroundTime << endl << endl;
	}
